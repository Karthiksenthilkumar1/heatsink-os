...
class ConfigManager(QObject):
    ...
    def __init__(self, config_dir="config"):
        ...
        self.baseline_stats = {
            "avg_temp": 0.0,
            "peak_temp": 0.0,
            "stability_score": 0.0, # Average absolute deviation from mean
            "data_points": 0
        }
        self.session_history = [] # Rolling 100 points for current session
        self.optimization_steps = 0 # Count of migrations performed
    ...
    def update_performance_metrics(self, current_avg_temp, current_max_temp):
        ...
        if self.baseline_stats["data_points"] < 60:
            ...
            # Simple stability: average absolute deviation during baseline
            dev = abs(current_avg_temp - self.baseline_stats["avg_temp"])
            self.baseline_stats["stability_score"] = (self.baseline_stats["stability_score"] * n + dev) / (n + 1)
    ...
    def get_performance_snapshot(self):
        ...
        # Stability: current avg deviation
        curr_dev = sum(abs(x - curr_avg) for x in self.session_history) / len(self.session_history)
        
        # Improvement deltas
        temp_reduction = self.baseline_stats["avg_temp"] - curr_avg
        peak_reduction = self.baseline_stats["peak_temp"] - curr_peak
        stability_gain = self.baseline_stats["stability_score"] - curr_dev
        
        is_ready = self.baseline_stats["data_points"] >= 60
        progress = (self.baseline_stats["data_points"] / 60.0) * 100
        ...
        return {
            ...
            "temp_reduction": temp_reduction, # Delta might be negative early on
    ...
    def add_history_entry(self, decision):
        ...
        entry = {
            ...
            "from_type": decision.get("from_type", "Unknown"),
            "to_type": decision.get("to_type", "Unknown"),
            "selection_reason": decision.get("selection_reason", "Priority-based")
        }
