...
class CoreDetailView(QFrame):
    ...
    def set_core(self, core_id, history_data=[]):
        ...
        # Determine core type for title
        ctype = "P-CORE" if core_id < 4 else "E-CORE"
        self.title.setText(f"CORE {core_id:02d} ({ctype}) DETAILS")
        self.history = []
        self.curve.setData([])
        ...
        if relevant:
            ...
            for r in relevant[:5]:
                ...
                reason = r.get('selection_reason', 'Load Balancing')
                txt += f"• [{marker}] {r['process_name']} via {reason}\n"
            self.event_list.setText(txt)
        else:
            self.event_list.setText("No recent migration events.")

    def update_data(self, temp, load_metric, top_processes=[], trend=None, prediction=None, fatigue=0):
        load = load_metric.get("load_percent", 0.0) if isinstance(load_metric, dict) else load_metric
