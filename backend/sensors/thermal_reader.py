...
class ThermalSensor:
    ...
    def get_thermal_data(self):
        ...
        if not data["cores"]:
            ...
            data["CPU_Package"]["temperature"] = 55.0 + random.uniform(-1, 5) # Real-world base idle
            data["CPU_Package"]["source"] = "SIMULATED"
            
            # Infer cooling efficiency (1.0 = perfect, lower = heat soak)
            # This would normally be calculated by comparing Power vs Temp Rise
            data["cooling_efficiency"] = random.uniform(0.7, 0.95)
            
            for i in range(total_cores):
                # Simple rule: Cores 0-3 are P-Cores, others are E-Cores (standard hybrid arch)
                core_type = "P-Core" if i < 4 else "E-Core"
                data["cores"][i] = {
                    "temperature": 50.0 + random.uniform(-2, 10),
                    "unit": "C",
                    "core_type": core_type,
                    "timestamp": time.time()
                }
